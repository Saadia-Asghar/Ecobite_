...
import { Users, Package, LogOut, Award, Download, Trash2, DollarSign, Plus, Pause, Play, Eye, X, Pencil, FileText, MapPin, Settings, Megaphone, ExternalLink, Check, CheckCircle, Shield, ShieldCheck, Activity, Truck, Info, Image, RefreshCw, Maximize2 } from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    const handleToggleVerify = async (userId: string, currentStatus: boolean, userName: string) => {
        try {
            ...
            if (response.ok) {
                setUsers(prev => prev.map(u => u.id === userId ? { ...u, isVerified: !currentStatus ? 1 : 0 } : u));
                await logAction('VERIFY_USER', userId, `${!currentStatus ? 'Verified' : 'Unverified'} user: ${userName}`);
                                ...
                                <tbody className="divide-y divide-forest-100 dark:divide-forest-700">
                                    {users.filter(u =>
                                        ...
                                        <tr key={user.id} className="hover:bg-forest-50 dark:hover:bg-forest-700/50">
                                            <td className="p-3">
                                                <span className="font-medium text-forest-900 dark:text-ivory flex items-center gap-1">
                                                    {user.name}
                                                    {user.isVerified === 1 && <CheckCircle className="w-3 h-3 text-mint fill-mint" />}
                                                </span>
                                            </td>
                                            ...
                                            <td className="p-3">
                                                <div className="flex gap-2">
                                                    <button
                                                        onClick={() => handleToggleVerify(user.id, user.isVerified === 1, user.name)}
                                                        className={`p-2 rounded-lg transition-colors ${user.isVerified === 1 ? 'bg-mint/40 text-forest-900' : 'bg-gray-100 dark:bg-forest-700 text-forest-400'}`}
                                                        title={user.isVerified === 1 ? 'Unverify' : 'Verify'}
                                                    >
